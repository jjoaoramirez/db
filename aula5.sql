...
SELECT idcliente, nome, UPPER(nome) as nomeMaiusculo, LOWER(nome) as nomeMinusculo
FROM CLIENTE;

SELECT idcliente, nome, CONCAT(idcliente, ' - ',nome) NomeConcatenado
FROM CLIENTE;

SELECT * FROM CLIENTE

SELECT idcliente, nome, email, CONCAT(idcliente, ' - ', nome, ' <', email, '>') Ficha
FROM CLIENTE;

SELECT nome,
SUBSTRING(nome, 5, 4) as parte,
LEFT (nome, 3) as inicio,
RIGHT(cpf, 4) as final_cpf
FROM CLIENTE;

SELECT nome, CHAR_LENGTH(nome) as qtd_caracteres 
FROM CLIENTE;

INSERT INTO CLIENTE (idcliente, nome, cpf, email, status)
VALUES
(10, ' Ana Flávia ', '123456', 'teste@gmail.com', 'Ativo');

SELECT nome, TRIM(nome), RTRIM(nome), LTRIM(nome)
FROM CLIENTE
WHERE idcliente = 10;

SELECT LPAD(idproduto, 5, '0') as codigo, nome FROM PRODUTO;

SELECT idpedido, YEAR(data_pedido) as ano, MONTH(data_pedido) as mes, DAY(data_pedido) as dia FROM PEDIDO;

SELECT idpedido, DATE_FORMAT(data_pedido, '%d/%m/%y') as data_br FROM PEDIDO;

SELECT idpedido, DATEDIFF(CURDATE(), data_pedido) as dias_atras FROM PEDIDO;